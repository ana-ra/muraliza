            ...
            List {
                ...
                Section {
                    ...
                    .onChange(of: isEditing) {
                        if isEditing == true {
                            withAnimation {
                                proxy.scrollTo("artist", anchor: .top)
                            }
                        }
                    }
                }
                ...
                if !artistSearchResults.isEmpty {
                    Section {
                        ForEach(0..<min(artistSearchResults.count, 4), id: \.self) { index in
                            HStack {
                                Text(artistSearchResults[index])
                                Spacer()
                            }
                            .onTapGesture {
                                artist = artistSearchResults[index]
                                clearSearchList = true
                            }
                            .listRowBackground(Color.clear)
...
#Preview {
    ...
    NewWorkView(colaborationViewModel: colaborationViewModel)
        ...
        .onAppear {
            colaborationViewModel.image = UIImage(resource: .ima)
        }
}
